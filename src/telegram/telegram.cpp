...
AFuture<AJson> telegram::longPoll() {
    ...
    auto r = co_await builder.runAsync();
...
AFuture<telegram::message_id_t> telegram::postMessage(telegram::Message message) {
    ...
    auto response = co_await builder.runAsync();
    ...
    co_return AJson::fromBuffer(response.body)["result"]["message_id"].asLongInt();
}
...
AFuture<> telegram::answerCallbackQuery(AString queryId) {
    co_await ACurl::Builder(formatMethodUrl("answerCallbackQuery"))
